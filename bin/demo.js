...
Main.main = function() {
		...
		return thx_Maps.tuples(thx_Arrays.groupByAppend(cards1,groupBy[0],new haxe_ds_StringMap())).map(function(tuple) {
			...
			if(restOfGroupBys.length == 0) {
				var array = tuple._1.map(function(card) {
					...
					var textContent = card.name;
					if(null != textContent) {
						el2.appendChild(doc1.createTextNode(textContent));
					}
					var tmp2 = el2;
					var doc2 = null;
					if(null == doc2) {
					...
					if(null != children1) {
						var _g32 = 0;
						while(_g32 < children1.length) {
							var child1 = children1[_g32];
							++_g32;
							el4.appendChild(child1);
						}
					}
					var textContent1 = null;
					if(null != textContent1) {
						el4.appendChild(doc3.createTextNode(textContent1));
					}
					var children2 = [el4];
					if(null != children2) {
						var _g33 = 0;
						while(_g33 < children2.length) {
							var child2 = children2[_g33];
							++_g33;
					...
					var children3 = [tmp2,el3];
					if(null != children3) {
						var _g24 = 0;
						while(_g24 < children3.length) {
							var child3 = children3[_g24];
							++_g24;
							el1.appendChild(child3);
						}
					}
					var textContent3 = null;
					if(null != textContent3) {
						el1.appendChild(doc.createTextNode(textContent3));
					}
					return [el1,fancy_table_util__$CellContent_CellContent_$Impl_$.fromString(Std.string(card.cmc)),fancy_table_util__$CellContent_CellContent_$Impl_$.fromString(Std.string(card.draftval)),fancy_table_util__$CellContent_CellContent_$Impl_$.fromString(Std.string(card.tcgprice))];
				});
				...
				if(null != children4) {
					var _g5 = 0;
					while(_g5 < children4.length) {
						var child4 = children4[_g5];
						++_g5;
						el5.appendChild(child4);
					}
				}
				var textContent4 = tuple._0;
				if(null != textContent4) {
					el5.appendChild(doc4.createTextNode(textContent4));
				}
				return { values : [el5], data : cardsToRowData(tuple._1,restOfGroupBys)};
			}
		});
	};
	...
	var table = new fancy_Table(el,{ data : fancy_table_util_FancyTableData.Nested((function(cards2) {
		...
		if(null != children5) {
			var _g7 = 0;
			while(_g7 < children5.length) {
				var child5 = children5[_g7];
				++_g7;
				el6.appendChild(child5);
			}
		}
		var textContent5 = "Cards";
		...
		var el7 = doc6.createElement("span");
		var _g8 = 0;
		var _g15 = [];
		...
		if(null != children6) {
			...
			while(_g9 < children6.length) {
				var child6 = children6[_g9];
				++_g9;
				el7.appendChild(child6);
			}
		}
		var textContent6 = "CMC";
		...
		var attrs7 = null;
		if(null != attrs7) {
			var tmp10 = attrs7.keys();
			while(tmp10.hasNext()) {
				var attr7 = tmp10.next();
		...
		if(null != children7) {
			var _g17 = 0;
			while(_g17 < children7.length) {
				var child7 = children7[_g17];
				++_g17;
				el8.appendChild(child7);
			}
		}
		var textContent7 = "Draft Value";
		...
		var el9 = doc8.createElement("span");
		var _g18 = 0;
		var _g19 = [];
		while(_g18 < _g19.length) {
			var o8 = _g19[_g18];
			++_g18;
			el9.setAttribute(o8.name,o8.value);
		}
		var attrs8 = null;
		if(null != attrs8) {
			var tmp12 = attrs8.keys();
			while(tmp12.hasNext()) {
				var attr8 = tmp12.next();
				el9.setAttribute(attr8,__map_reserved[attr8] != null?attrs8.getReserved(attr8):attrs8.h[attr8]);
			}
		}
		var children8 = null;
		if(null != children8) {
			var _g20 = 0;
			while(_g20 < children8.length) {
				var child8 = children8[_g20];
				++_g20;
				el9.appendChild(child8);
			}
		}
		var textContent8 = "Price";
		if(null != textContent8) {
			el9.appendChild(doc8.createTextNode(textContent8));
		}
		return [{ values : [tmp7,tmp9,tmp11,el9]}].concat(cardsToRowData1(cards2,[function(card1) {
			return card1.color;
		},function(card2) {
			return card2.rarity;
		},function(card3) {
...
var fancy_Table = function(parent,options) {
	...
	this.grid = new fancy_Grid(parent,{ rows : this.rows.length, columns : this.rows[0].cells.length, render : $bind(this,this.renderGrid)});
};
...
fancy_Table.prototype = {
	...
	,renderGrid: function(row,col) {
		return this.rows[row].cells[col].el;
	}
	,setData: function(data) {
		switch(data[1]) {
		case 0:
			return this.setTabularData(data[2]);
		case 1:
			return this.setNestedData(data[2]);
		}
	}
	,setTabularData: function(data) {
		var tmp = this.empty();
...
var fancy_table_Cell = function(value,fixed,onclick) {
	...
	var el = doc.createElement("div");
	var _g = 0;
	var _g1 = [{ value : "ft-cell", name : "class"}];
	while(_g < _g1.length) {
		var o = _g1[_g];
		++_g;
		el.setAttribute(o.name,o.value);
...
fancy_table_Row.prototype = {
	...
	,createRowElement: function(children) {
		...
		return thx_Arrays.reduce(classes,dots_Dom.addClass,el);
	}
...
var fancy_table_util_FancyTableData = { __ename__ : true, __constructs__ : ["Tabular","Nested"] };
fancy_table_util_FancyTableData.Tabular = function(data) { var $x = ["Tabular",0,data]; $x.__enum__ = fancy_table_util_FancyTableData; $x.toString = $estr; return $x; };
